  ...
  <v-app>
    <v-main class="d-flex flex-column pt-7">
      ...
      >
        ...
        <v-spacer />
        <v-btn :disabled="store.isExternalRequest" icon @click="selectMultiFile">
          <v-icon icon="mdi-image-multiple" />
          <v-tooltip activator="parent" location="bottom">Load multiple images</v-tooltip>
        </v-btn>
        <v-btn :disabled="store.isExternalRequest" icon @click="addTab">
          <v-icon icon="mdi-plus" />
          <v-tooltip activator="parent" location="bottom">Add new image tab</v-tooltip>
        </v-btn>
        <v-btn v-if="store.hasGridScoreImage" :disabled="store.isExternalRequest" icon @click="exportToGridScore">
          <v-icon>
            <svg
              class="b-icon bi"
              fill="currentColor"
        ...
        <v-btn :disabled="store.isExternalRequest" icon @click="downloadData">
          <v-icon icon="mdi-file-excel-outline" />
          <v-tooltip activator="parent" location="bottom">Export data to Excel</v-tooltip>
        </v-btn>
        <v-btn :disabled="store.isExternalRequest" icon @click="showSettings">
          <v-icon icon="mdi-cog" />
    ...
    >
      <v-card
        ...
        text="An updated version of Spriggles is available. Please use the button below to load the new content and refresh the page."
      >
        <template #actions>
          <v-btn text="Cancel" variant="text" @click="updateExists = false" />
          <v-btn text="Update" variant="tonal" color="primary" @click="refreshApp" />
        </template>
      </v-card>
    </v-dialog>
  </v-app>
